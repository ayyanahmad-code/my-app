        ...
        >
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <motion.div variants={slideInLeftVariants}>
              ...
              >
                <Box size={16} />
                <span className="text-sm font-semibold">DIGITAL RECONSTRUCTION</span>
              </motion.div>
              <motion.h2 
                className="text-4xl font-bold text-secondary-800 mb-6"
                variants={itemVariants}
              >
                From Physical to Digital Precision
              </motion.h2>
              <motion.div className="space-y-4 text-lg text-gray-600" variants={containerVariants}>
                <motion.p variants={itemVariants}>
                  We specialize in converting physical models, legacy parts, or image files 
                  into accurate, editable digital models. Our reverse engineering services 
                  help breathe new life into existing products and enable modernization.
                </motion.p>
                <motion.p variants={itemVariants}>
                  Whether you need to recreate obsolete parts, analyze competitor products, 
                  or enhance existing designs, our reverse engineering expertise delivers 
                  precise digital models ready for modification, analysis, or reproduction.
                </motion.p>
              </motion.div>
              ...
              <motion.div 
                ...
                whileHover="hover"
              >
                <h4 className="font-bold text-secondary-800 mb-4">Common Scenarios:</h4>
                <div className="space-y-3">
                  {[
                    { scenario: 'Obsolete Part Recreation', icon: '🔄' },
                    { scenario: 'Competitive Product Analysis', icon: '🔍' },
                    { scenario: 'Legacy Equipment Support', icon: '🏭' },
                    { scenario: 'Damage Assessment & Repair', icon: '🔧' },
                    { scenario: 'Design Improvement Studies', icon: '📈' },
                    { scenario: 'Manufacturing Process Analysis', icon: '⚙️' }
                  ].map((item, index) => (
                    <motion.div 
                      key={index} 
            ...
            >
              <div className="flex items-center mb-4">
                ...
                <h3 className="text-2xl font-bold">Tools & Accuracy</h3>
              </div>
              <p className="mb-6">
                Using industry-leading tools like SolidWorks with advanced scanning 
                technologies, we ensure accuracy and integrity in every digital 
                reconstruction project.
              </p>
              
              <div className="space-y-4">
                {[
                  { text: 'High-precision 3D scanning', icon: '🎯' },
                  { text: 'Photogrammetry for complex shapes', icon: '📸' },
                  { text: 'Parametric CAD modeling', icon: '📐' },
                  { text: 'Dimensional accuracy verification', icon: '📊' }
                ].map((item, index) => (
                  <motion.div 
                    key={index} 
